...
Hi Tyler, The 2019 analysis is fairly straightforward but it does change how we interpret the 2017 preliminary write up we worked on previously. In short, some of the assumptions I felt were OK back then I'm not comfortable with now. Please take a look and then let’s discuss.
  
# Abundance estimate  
  
## Size Selective Sampling Tests  
With respect to size there is no evidence of selective sampling during either event. None of these plots look great to me but the test statistic is insignificant for all three tests. These tests all point toward used of a pooled Peterson estimator.
```{r echo = FALSE}
...
## ASL at the weir
ASL from the weir is pretty straightforward. Timing of the ASL samples was not representative of emigration in general...
```{r}
...
For the spawning steelhead ASL we are assuming a single representative grab sample.
```{r}
...
## Spawning survival
The first row of numbers is the estimate, the second row of numbers is the SE.
```{r}
#Spawning survival
